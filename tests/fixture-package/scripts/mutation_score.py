...
def _parse_args(argv: Sequence[str] | None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--stats", required=True, type=Path, help="mutmut-cicd-stats.json"
    )
    parser.add_argument(
        "--baseline", required=True, type=Path, help=".mutation-baseline"
    )
    parser.add_argument(
        "--summary", type=Path, help="GitHub step-summary file to append"
    )
    parser.add_argument("--label", default="", help="scope label, e.g. 'changed files'")
    parser.add_argument(
        "--shards", type=Path, help=".mutation-shards.json (with --shard)"
    )
