...
The specification for the Portolan brand kit: a single self-contained `brand/` folder holding hand-edited SVG, hand-edited JSON, and two small stdlib-only Python scripts. No build step, no external dependencies. Adapted from the Radiant Earth ops-re brand kit pattern.
...
| `logos` | yes | Slot names to relative paths. Required slots: `full` (horizontal lockup) and `mark` (square). Color variants use hex-suffixed filenames (`portolan-logo-26383a.svg`); dark-surface variants take a `_dark` slot suffix; a `currentcolor` variant serves inline SVG. |
...
Keys starting with `_` are documentation comments; tools ignore them. `_stub: true` marks the kit as unpopulated: `check.py` passes trivially and `emit_css.py` refuses to run.
