...
Single source of truth for pip-audit ignores, consumed by the security
job in the reusable Python CI:
...
Entries past their expiry date are dropped, so a still-present
vulnerability starts failing CI again automatically instead of being
ignored forever. Malformed lines fail loudly — a broken ignore file
must never silently disable auditing.
"""
    ...
    for lineno, raw in enumerate(path.read_text().splitlines(), start=1):
        ...
        if len(parts) < 3:
            raise IgnoreFileError(
                f"{path.name} line {lineno}: expected "
                f"'VULN-ID EXPIRES REASON', got: {raw!r}"
            )
        ...
        try:
            ...
            raise IgnoreFileError(
                f"{path.name} line {lineno}: expiry must be "
                f"YYYY-MM-DD, got: {expires_str!r}"
            ) from exc
        if today <= expires:
            ids.append(vuln_id)
    return ids
...
def main() -> int:
    try:
        ...
        ids = active_ignores(DEFAULT_PATH, today=today)
